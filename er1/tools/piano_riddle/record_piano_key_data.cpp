...
static int  cfg_post_ms = 400;
static int  cfg_lead_ms = 150;
...
static bool capture_event_now(){
  ...
  // rough estimate: big header + rows
  size_t est = 1400 + (size_t)(WANT_PRE_FRAMES + WANT_POST_FRAMES + 16) * (size_t)(80 + 12 * N_BANDS128);
  if (!ensure_space_bytes(est)) {
    Serial.printf("ERR: not enough SPIFFS space for capture (need ~%u bytes). Use purge.\n", (unsigned)est);
    fsinfo();
    return false;
  }
  ...
  int row_idx = 0; // counts all written rows (pre+post)

  // synthetic missing pre frames
  ...
  for(int f=1; f<=WANT_POST_FRAMES; f++){
    ...
    // onset detection only after lead region begins
    ...
    // clip + snr proxy (flatness) in deterministic window
    clip_sum += (double)clip;
    clip_n++;

    if (ms >= 0 && ms <= 180) {
      flat_sum += (double)r.flatness;
      flat_n++;
    }
  ...
  // --- overwrite DIAG line (fixed width) ---
  char diag[256];
  ...
  out.seek(diag_pos);
  out.print(diag);
  // pad to ensure we overwrite the reserved spaces cleanly
...
static void print_help() {
  ...
  Serial.println("  stop");
  Serial.println("  redo               (NOT supported in dump-per-rep mode)");
  Serial.println("");
...
static void process_line(String line) {
  ...
  if (cmd == "help" || cmd == "?") {
    ...
    Serial.println("ERR: redo not supported when dumping after each rep (rep already emitted).");
  } else {
...
void setup() {
  ...
  // Allocate big buffers on heap (NOT .bss)
  ...
  // ring storage allocated later by ring_alloc_for_ms()
