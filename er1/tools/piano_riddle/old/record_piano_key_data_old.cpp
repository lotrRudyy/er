...
static const int QUIET_FRAMES_REQUIRED = 10; // consecutive frames near baseline
...
static void measure_baseline_silence_5s(){
  ...
  while (millis() - t0 < dur_ms) {
    ...
    // do NOT disturb x_win/pre-ring too much; but we do want realistic input, so keep window updated
    push_window(hop_f);
    sum += (double)rms;
...
static void tick_cooldown(){
  ...
  if (collect_mode == MODE_KEYS_NOISE) {
    if (now >= cooldown_until_ms) cooldown_active = false;
    return;
  }
...
static void print_help() {
  ...
  Serial.println("  start <key> <reps>           (collect keys clean; baseline 5s; cooldown until quiet or 5s cap)");
...
static void tick_audio_features(){
  ...
  // NEW: tick cooldown state machine off the live RMS stream
...
void loop() {
  ...
  if (enter_trigger){
    ...
    // NEW: cooldown gate blocks captures
    if (cooldown_active) {
      uint32_t now = millis();
    ...
    if (ok) {
      // NEW: start cooldown after each capture
