...
static const char* FW_DESC = "maglock game_state plus cmd";
...
static void maglockCommandSubscription(NodeContext& ctx, const char* topic, const String& payload, void* user) {
  ...
  (void)topic;
  auto* module = static_cast<MaglockController*>(user);
  if (module) module->onMaglockCommandMessage(payload);
}
...
void setup() {
  ...
  nodeCore.registerSubscription(TOPIC_GAME, gameStateSubscription, &maglock);
