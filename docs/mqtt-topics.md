...
- Nodes are the device identities (e.g., `maglock`, `images`, `piano`, `chess`, `stop_timer`, `star_slider`, `star_sky`, `knocking`, `candles`).
- Images/Piano split: firmware exposes two logical nodes on one MCU: `images/<channel>` and `piano/<channel>`.
