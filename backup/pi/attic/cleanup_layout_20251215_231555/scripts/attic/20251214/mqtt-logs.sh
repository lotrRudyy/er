#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="$ROOT_DIR/logs"
BROKER="${LOCAL_BROKER:-127.0.0.1}"
LOCK_FILE="$LOG_DIR/mqtt-logs.pid"

ensure_log_dir() {
  mkdir -p "$LOG_DIR"
}

log_file_for_date() {
  local date_part="$1"
  echo "$LOG_DIR/er1-${date_part}.log"
}

timestamp() {
  date +"[%d.%m.%Y %H:%M:%S.%3N]"
}

acquire_lock() {
  ensure_log_dir
  if [[ -f "$LOCK_FILE" ]]; then
    local oldpid
    oldpid="$(cat "$LOCK_FILE" 2>/dev/null || true)"
    if [[ -n "$oldpid" && -d "/proc/$oldpid" ]]; then
      echo "mqtt-logs.sh daemon already running (pid=$oldpid). Exiting." >&2
      exit 1
    fi
  fi
  echo "$$" > "$LOCK_FILE"
}

release_lock() {
  rm -f "$LOCK_FILE" 2>/dev/null || true
}

write_stream_daemon() {
  ensure_log_dir

  local current_date file today stamped
  current_date="$(date +%d.%m.%Y)"
  file="$(log_file_for_date "$current_date")"

  mosquitto_sub -h "$BROKER" -t 'esc/#' -v | while IFS= read -r line; do
    today="$(date +%d.%m.%Y)"
    if [[ "$today" != "$current_date" ]]; then
      current_date="$today"
      file="$(log_file_for_date "$current_date")"
    fi

    stamped="$(timestamp) $line"
    echo "$stamped" >> "$file"
  done
}

tail_today() {
  ensure_log_dir
  local file
  file="$(log_file_for_date "$(date +%d.%m.%Y)")"
  touch "$file"
  tail -f "$file"
}

grep_today() {
  ensure_log_dir
  [[ $# -ge 1 ]] || { echo "Pattern required for grep command." >&2; exit 1; }

  local pattern file
  pattern="$*"
  file="$(log_file_for_date "$(date +%d.%m.%Y)")"

  if [[ ! -f "$file" ]]; then
    echo "No logfile found for today: $file" >&2
    exit 1
  fi

  grep --color=auto "$pattern" "$file"
}

print_help() {
  cat <<'EOF'
Usage: scripts/mqtt-logs.sh <command> [args]

Commands:
  daemon          Subscribe to esc/# and write to /home/rudyy/er1/logs/er1-DD.MM.YYYY.log (no stdout).
  live            tail -f today's logfile (requires daemon to be running).
  tail            tail -f today's logfile.
  grep <pattern>  Search today's logfile for <pattern>.
  help            Show this help text.
Notes:
  - Broker is taken from $LOCAL_BROKER (default: 127.0.0.1).
  - Log timestamp uses: date +"[%d.%m.%Y %H:%M:%S.%3N]".
EOF
}

main() {
  local cmd="${1:-help}"
  shift || true

  case "$cmd" in
    daemon)
      acquire_lock
      trap release_lock EXIT
      write_stream_daemon
      ;;
    live)
      # no second mosquitto_sub; just follow what daemon writes
      tail_today
      ;;
    tail)
      tail_today
      ;;
    grep)
      grep_today "$@"
      ;;
    help|-h|--help)
      print_help
      ;;
    *)
      echo "Unknown command: $cmd" >&2
      print_help
      exit 1
      ;;
  esac
}

main "$@"
