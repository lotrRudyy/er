...
candle_angles = [334.425, 246.379, 130.303, 54.2855];
function candle_ang(label) = candle_angles[label];

// Zones (master local angles, non-touching)
box_local_angles = [83.9371, 49.5987, 292.977, 252.763];
...
function nail_ang(i) = [139.746, 50.1615, 290.612, 208.639][i];

// top rotation aligns top line (local +Y) to nail direction
function top_rot_for_disk(i) = [294.171, 0.46469, 164.897, 275.019][i];
...
// ============================================================
// Hard requirements:
// - ONE physical token disk -> SAME wall marks for all 4 placements
// - For each hole i, after applying THAT hole's top rotation, the TOTAL number of marks
//   (bottom-wall + top-wall) inside the CORRESPONDING COLOR ZONE (owned by hole i)
//   equals zone_solution_counts[i].
// - No "on the border": each mark's CENTER must be far enough from the zone edge so that
//   at least HALF the mark thickness is clearly inside/outside, plus a user margin.
// - Must NEVER hang/freeze OpenSCAD: bounded attempts, no unbounded brute force.
...
// Bounded attempts prevent hangs; deficits compensated on BOTTOM.
// NOTE: use wall_mark_min_sep_deg_inside so 4 marks can fit in a zone.
...
global_bottom_marks = [92.2362, 40.8802, 55.5225, 50.3159, 258.546, 250.841, 246.945, 129.35, 141.644, 194.483, 333.372, 325.721, 186.199, 217.578, 210.179, 153.336, 353.209, 201.052, 8.72721, 122.153];
...
global_top_marks = [158.958, 48.2201, 118.696, 82.0312, 212.456, 253.036, 186.202, 274.864, 13.6619, 97.5625, 204.588, 264.389, 227.571, 287.806, 220.341, 90.7055, 243.061, 236.142, 193.614, 304.053];
...
// Disk: base gray + 4 colored sectors (full height => walls colored too)
